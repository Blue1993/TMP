...
class MoleculeGraphDataset(Dataset):
    def __init__(self, root, device = 'cuda:0', processed_file = None,
            transform = None, pre_transform = None, pre_filter = None):
        self.root = root 
        self.processed_file = processed_file
        self.device = device

        if not os.path.exists(f"{self.root}/processed/{self.processed_file}"):
            print("Preparing data ...")
            self.process()
        else:
            print("File exists pass preprocessing ...")
            print("Load preprocessed data ...")
            data_dict = torch.load(f"{self.root}/processed/{self.processed_file}")
    ...
    def process(self):
        data_dict = torch.load(f"{self.root}/raw/{self.processed_file}")
        torch.save(data_dict, f"{self.root}/processed/{self.processed_file}") 
