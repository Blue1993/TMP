...
def main():
    ...
    for idx in range(1, 2):
        ...
        with open(f"../results/pre-training/protein/CV{idx}/BA_Training_pocket.pkl", "rb") as f:
            BA_Training_results_dict = pickle.load(f)
            
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(BA_training_ID, BA_training_BS['4A'], BA_Training_results_dict, BA_training_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        ...
        print(f"[pos len]: {pos_len}")
        print()
            
        with open(f"../results/pre-training/protein/CV{idx}/CASF2016_pocket.pkl", "rb") as f:
            CASF2016_results_dict = pickle.load(f)  
            
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(CASF2016_ID, CASF2016_BS['4A'], CASF2016_results_dict, CASF2016_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/CASF2013_pocket.pkl", "rb") as f:
            CASF2013_results_dict = pickle.load(f)
        
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(CASF2013_ID, CASF2013_BS['4A'], CASF2013_results_dict, CASF2013_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/CSAR2014_pocket.pkl", "rb") as f:
            CSAR2014_results_dict = pickle.load(f)
        
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(CSAR2014_ID, CSAR2014_BS['4A'], CSAR2014_results_dict, CSAR2014_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/CSAR2012_pocket.pkl", "rb") as f:
            CSAR2012_results_dict = pickle.load(f)
        
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(CSAR2012_ID, CSAR2012_BS['4A'], CSAR2012_results_dict, CSAR2012_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/CSARset1_pocket.pkl", "rb") as f:
            CSARset1_results_dict = pickle.load(f)
            
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(CSARset1_ID, CSARset1_BS['4A'], CSARset1_results_dict, CSARset1_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/CSARset2_pocket.pkl", "rb") as f:
            CSARset2_results_dict = pickle.load(f)
            
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(CSARset2_ID, CSARset2_BS['4A'], CSARset2_results_dict, CSARset2_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/Astex_pocket.pkl", "rb") as f:
            Astex_results_dict = pickle.load(f) 
            
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(Astex_ID, Astex_BS['4A'], Astex_results_dict, Astex_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/COACH420_pocket.pkl", "rb") as f:
            COACH420_results_dict = pickle.load(f) 
            
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(COACH420_ID, COACH420_BS['4A'], COACH420_results_dict, COACH420_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
        ...
        with open(f"../results/pre-training/protein/CV{idx}/HOLO4K_pocket.pkl", "rb") as f:
            HOLO4K_results_dict = pickle.load(f) 
            
        #BA_training_results = get_SR_results(training_ID, training_BS['4A'], DTATraining_predictions_dict[0], train_lengths)
        precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_len, pos_len = get_another_metric_results(HOLO4K_ID, HOLO4K_BS['4A'], HOLO4K_results_dict, HOLO4K_lengths)
        print(f"[Precision]: {precision:.4f}")
        print(f"[Recall]: {recall:.4f}")
        print(f"[Accuruacy]: {acc:.4f}")
        print(f"[Sensitivity]: {sensitivity:.4f}")
...
def get_another_metric_results(IDs, labels, predictions, lengths):
    #precision, recall, sensitivity, specificity = list(), list(), list(), list()
    total_TP, total_TN, total_FP, total_FN, total_lengths, positive_lengths = 0, 0, 0, 0, 0, 0
    
    for idx, id_ in enumerate(IDs):
        total_indexes = [i for i in range(lengths[idx])]
        pocket_labels = list(map(int, labels[idx].split(",")))
        
        positive_predictions = remove_over_lengths(predictions[idx], lengths[idx], 0.55)
        #positive_predictions = remove_over_lengths(predictions[id_], lengths[idx], 0.55)
        ...
        #print(TP, TN, FP, FN, lengths[idx])
        
        total_TP += TP
        total_TN += TN
        total_FP += FP
        total_FN += FN
        total_lengths += lengths[idx]
        positive_lengths += len(positive_predictions)
    ...
    return precision, recall, acc, sensitivity, specificity, gmean, F2, F1, total_lengths/len(IDs), positive_lengths/len(IDs)
