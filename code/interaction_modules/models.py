...
class ProteinEncoderModule(nn.Module):
    def __init__(self, config, device):
        ...
        self.linear = nn.Linear(self.config['Architecture']['hidden_size'] * 4, self.config['Architecture']['hidden_size'])
...
class PretrainedBA(nn.Module):
    ...
    def forward(self, protein_data, compound_graphs, pocket_mask, compound_mask):
        ...
        # get cross features
        resi_rep, node_rep = self.cross_encoder(resi_rep, pocket_extend_mask, node_rep, compound_extended_attention_mask, pocket_mask, compound_mask) 
...
class InteractionSitesModule(nn.Module):
    ...
    def forward(self, protein_features, pocket_mask, compound_features, compound_mask, pairwise_mask):
        ...
        latent_vectors_mean = torch.sum(latent_features.view(latent_features.size()[0], latent_features.size()[1] * latent_features.size()[2], -1), dim = 1)
        ...
        return latent_vectors_mean, pairwise_map
